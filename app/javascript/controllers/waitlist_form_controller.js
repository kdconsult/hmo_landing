  ...
  updateSpotsUI() {
    ...
    if (this.progressBar) {
      const percent = Math.max(
        0,
        Math.min(100, (spotsRemaining / MAX_SPOTS) * 100)
      );
      ...
      this.progressBar.classList.toggle(
        "animate-pulse-bar",
        spotsRemaining <= 10
      );
      if (spotsRemaining > 10)
        this.progressBar.classList.remove("animate-pulse-bar");
    }
    if (this.spotsMessage) {
      if (spotsRemaining <= 10 && spotsRemaining > 0) {
        ...
        this.spotsMessage.textContent =
          "Всички места за ранно достъпване са заети. Присъединете се към списъка за изчакване за шанс!";
        this.spotsMessage.className = "text-red-600 font-bold mt-2";
      } else {
        this.spotsMessage.textContent = "";
      }
    ...
    if (this.fomoDiv) {
      if (spotsRemaining <= 10 && spotsRemaining > 0) {
        ...
        this.fomoDiv.textContent =
          "Всички места за ранно достъпване са заети. Все още можете да се присъедините към списъка за изчакване за шанс.";
      } else {
        this.fomoDiv.textContent = "";
      }
    }
  ...
  showLoadingState() {
    ...
    if (submitButton) {
      ...
      submitButton.innerHTML =
        '<span class="inline-block animate-spin mr-2">⏳</span> Обработване...';
    ...
    try {
      ...
      if (res.status === 422) {
        ...
        if (data.errors) {
          let errorMsg = "";
          if (
            data.errors.email_ciphertext &&
            data.errors.email_ciphertext.length > 0
          ) {
            errorMsg += data.errors.email_ciphertext.join(" ");
            this.showFieldError(
              this.form.querySelector('input[name="email"]'),
              data.errors.email_ciphertext[0]
            );
          }
          if (
            data.errors.phone_ciphertext &&
            data.errors.phone_ciphertext.length > 0
          ) {
            if (errorMsg) errorMsg += " ";
            errorMsg += data.errors.phone_ciphertext.join(" ");
            this.showFieldError(
              this.form.querySelector('input[name="phone"]'),
              data.errors.phone_ciphertext[0]
            );
          }
          if (
            data.errors.marketing_consent &&
            data.errors.marketing_consent.length > 0
          ) {
            if (errorMsg) errorMsg += " ";
...
window.waitlistCopyLink = function (shareUrl, event) {
  ...
  window.waitlistToast(
    "Линкът за покана е копиран! Споделете го с приятелите си.",
    "success"
  );
};
